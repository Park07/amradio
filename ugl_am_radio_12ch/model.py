...
class AppState:
    ...
    def __post_init__(self):
        if not self.channels:
            self.channels = [
                ChannelState(id=ch["id"], frequency=ch["default_freq"]) for ch in Config.CHANNELS
            ]
...
class Model:
    ...
    def set_broadcast(self, active: bool):
        ...
        active_count = sum(1 for ch in self.state.channels if ch.enabled)

        if active:
            self.logger.log(f"=== BROADCAST STARTED ({active_count} channels) ===", "INFO")
        else:
