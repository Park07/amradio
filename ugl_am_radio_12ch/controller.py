...
Updated: 12 channels with scrollable panel
"""
...
class Controller:
    ...
    def build_ui(self):
        ...
        with dpg.window(tag="main_window", label=""):
            ...
            # Channels section (scrollable for 12 channels)
    ...
    def _build_channels_section(self):
        """Build channel controls (12 channels in scrollable area)."""
        with dpg.group(horizontal=True):
            dpg.add_text("BROADCAST CHANNELS", color=Config.Colors.TEXT_DIM)
            dpg.add_spacer(width=10)
            dpg.add_text(f"({len(Config.CHANNELS)} channels)", color=Config.Colors.TEXT_DIM)

        dpg.add_spacer(height=8)

        # Scrollable child window for channels
        with dpg.child_window(tag="channels_scroll", height=300, border=True):
            for ch_config in Config.CHANNELS:
                ch_id = ch_config["id"]
                default_freq = ch_config["default_freq"]

                with dpg.child_window(tag=f"ch{ch_id}_panel", height=45, border=True):
                    with dpg.group(horizontal=True):
                        dpg.add_spacer(width=8)
                        ...
                        # Channel label
                        label = f"CH{ch_id}" if ch_id < 10 else f"CH{ch_id}"
                        dpg.add_text(label, tag=f"ch{ch_id}_label", color=Config.Colors.TEXT_DIM)
    ...
    def _build_log_section(self):
        ...
        with dpg.child_window(tag="log_window", height=120, border=True):
            dpg.add_text("System ready. 12-channel stress test mode.", tag="log_text", color=Config.Colors.LOG_INFO, wrap=460)
    ...
    def _build_confirm_popup(self):
        ...
        with dpg.window(
            ...
            dpg.add_spacer(height=8)
            dpg.add_text("", tag="confirm_channels", color=Config.Colors.TEXT_DIM, wrap=350)
    ...
    def _on_broadcast_click(self):
        ...
        if self.model.is_broadcasting():
            ...
            for ch in self.model.state.channels:
                if ch.enabled:
                    active_chs.append(f"CH{ch.id}:{ch.frequency // 1000}kHz")
            
            if active_chs:
                ch_text = f"Active channels ({len(active_chs)}): " + ", ".join(active_chs)
            else:
                ch_text = "Warning: No channels enabled!"
    ...
    def _on_state_change(self, state: AppState):
        ...
        # === BROADCAST BUTTON ===
        active_count = sum(1 for ch in state.channels if ch.enabled)
        ...
        if not state.connected:
            ...
            dpg.set_value("status_bar_text", f"BROADCASTING - {active_count} channels active")
            ...
            dpg.set_value("status_bar_text", f"Ready - {active_count} channels enabled")
